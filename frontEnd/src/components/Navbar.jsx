      ...
      <div className="container mx-auto max-w-6xl flex items-center justify-between">
        ...
        {isLoggedIn && (
          <div className="hidden md:flex space-x-8 items-center">
            <Link
              to="/dashboard"
              className="text-white font-semibold hover:text-yellow-300 transition-colors duration-200 text-lg transform hover:scale-110"
            >
              Dashboard
            </Link>
            <Link
        ...
        <div className="flex items-center space-x-4 relative">
          {isLoggedIn && (
            <span className="text-white mr-2 hidden md:inline font-semibold">
              {employeeName}
            </span>
          )}
          <div className="relative">
            ...
            {showDropdown && isLoggedIn && (
              <div className="absolute right-0 mt-3 bg-white rounded-lg shadow-xl p-2 w-48 z-50">
                <div className="block text-gray-800 font-semibold px-4 py-2 border-b border-gray-200">
                  {employeeName}
                </div>
                <Link
                  to={`/profile/${sessionStorage.getItem("creatorId")}`}
                  className="block text-gray-700 hover:bg-blue-100 px-4 py-2 rounded transition-colors duration-200"
                >
                  View Profile
                </Link>
                <button
      ...
      {isMenuOpen && isLoggedIn && (
        <div className="md:hidden mt-4 bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg rounded-lg shadow-lg p-4">
          <Link
            to="/dashboard"
            onClick={toggleMenu}
            className="block text-white font-semibold hover:text-yellow-300 py-2 transform hover:translate-x-2 transition-all duration-200"
          >
            Dashboard
          </Link>
          <Link
