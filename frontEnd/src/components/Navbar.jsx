  ...
  return (
    <nav className="bg-gradient-to-r from-gray-900 to-black p-4 shadow-lg">
      <div className="container mx-auto flex items-center justify-between">
        ...
        {isLoggedIn && (
          <>
            <Link to="/dashboard" className="text-white hover:text-sky-800 transition-colors duration-200">Dashboard</Link>
            <Link to="/create-meeting" className="text-white hover:text-sky-800 transition-colors duration-200">Schedule Meeting</Link>
            <Link to="/calendar" className="text-white hover:text-sky-800 transition-colors duration-200">Calendar</Link>
            <Link to="/search" className="text-white hover:text-sky-800 transition-colors duration-200">Search People</Link>
          </>
        )}

        {/* Profile Button */}
        <div className="flex items-center space-x-4 relative">
          {isLoggedIn && <span className="text-white mr-2">{employeeName}</span>}
      ...
      {isMenuOpen && isLoggedIn && (
        <div className="md:hidden mt-4 bg-gray-800 rounded-lg shadow-lg p-4">
          <Link to="/dashboard" onClick={toggleMenu} className="block text-white hover:text-gray-300 py-2">
            Dashboard
          </Link>
          <Link to="/create-meeting" onClick={toggleMenu} className="block text-white hover:text-gray-300 py-2">
            Schedule Meeting
          </Link>
          <Link to="/calendar" onClick={toggleMenu} className="block text-white hover:text-gray-300 py-2">
            Calendar
          </Link>
          <Link to="/search" onClick={toggleMenu} className="block text-white hover:text-gray-300 py-2">
            Search People
          </Link>
        </div>
      )}
